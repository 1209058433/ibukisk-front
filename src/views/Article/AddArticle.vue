...
export default {
  ...
  methods: {
    async getNextId() {
      ...
      this.form.id = res.nextId;
      this.addData.id = this.form.id;
    ...
    async save() {
      const { data: res } = await initData.post(
        ...
        qs.stringify({
          id: this.form.id,
          addForm: JSON.stringify(this.form),
        })
      );
    ...
    openTip() {
      const bb =
        "如果想使用特殊标签，则需要手动添加:<br><br>标题标签会默认添加2级标题，可以手动更改数字(1~6)<br>请在两个标签中间写内容<br><br>换行是手动增加一个空行，如果想两段相隔更远可以添加";
      this.$alert(bb, "我太菜了", {
